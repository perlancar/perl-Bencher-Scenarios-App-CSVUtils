...
Note that for stdin input (C<->), if reading is repeated via flow control, then
this hook will return the previously saved header row of stdin the first time
it is read. This is done because we can't seek stdin back to the beginning.
...
for stdin, the handle is not actually closed). Flow control is possible by
setting C<< $r-E<gt>{wants_repeat_file} >> to repeat reading the file or
C<< $r-E<gt>{wants_skip_files} >> to skip reading the rest of the files and go straight
to the C<after_close_input_files> hook.
...
the handle is not actually closed). Flow control is possible by setting
C<< $r-E<gt>{wants_repeat_files} >> to repeat reading all the input files.
...
If you are accepting CSV data, the following keys will also be available (in
C<on_input_header_row> and C<on_input_data_row> hooks):
